...
export default function Home() {
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#F0F2F5' }}>
      ...
      {/* ── HEADER ESCURO (GoodBoard-style) ── */}
      <header className="sticky top-0 z-10" style={{ backgroundColor: '#0A0A0A' }}>
        <div className="max-w-2xl mx-auto px-5 py-4 flex items-center justify-between">
          {/* Logo */}
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-xl flex items-center justify-center font-black text-sm" style={{ backgroundColor: LIME, color: '#0A0A0A' }}>
              C
            </div>
            <div>
              ...
              <span className="text-[10px] font-medium" style={{ color: '#6B7280' }}>Gestão financeira</span>
            </div>
          </div>

          {/* Month nav */}
          <div className="flex items-center gap-1 rounded-2xl px-1 py-1" style={{ backgroundColor: 'rgba(255,255,255,0.08)' }}>
            <button onClick={prevMonth} className="w-7 h-7 flex items-center justify-center rounded-xl transition-colors hover:bg-white/10"
              style={{ color: '#9CA3AF' }}>
            ...
            <button onClick={nextMonth} className="w-7 h-7 flex items-center justify-center rounded-xl transition-colors hover:bg-white/10"
              style={{ color: '#9CA3AF' }}>
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
        ...
        {/* Tab indicator strip */}
        <div className="max-w-2xl mx-auto px-5 flex gap-1 pb-3 overflow-x-auto scrollbar-none">
          {NAV_ITEMS.map(item => (
