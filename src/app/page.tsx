    ...
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="max-w-2xl mx-auto px-4 pt-4 pb-2">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-gray-900 text-base">Controle de Contas</span>
            </div>
            {/* Month nav */}
            <div className="flex items-center gap-1">
              <button onClick={prevMonth} className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-gray-100 transition-colors text-gray-500">
              ...
              <button onClick={nextMonth} className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-gray-100 transition-colors text-gray-500">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
      ...
      <main className="max-w-2xl mx-auto px-4 py-5 pb-24">
        {loading ? (
          ...
          <>
            {tab === 'dashboard' && <DashboardTab transactions={transactions} currentDate={currentDate} onRefresh={fetchAll} />}
        ...
        <div className="max-w-2xl mx-auto flex px-2 py-1">
          {TABS.map(t => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
              className={`flex-1 flex flex-col items-center gap-0.5 py-2 rounded-xl transition-colors ${tab === t.id ? 'text-indigo-600' : 'text-gray-400 hover:text-gray-600'}`}
            >
              {t.icon(tab === t.id)}
